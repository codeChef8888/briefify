...
def generate_telemetry_dataset(filename="account_telemetry.csv"):
    # Target Accounts with specific signals for Gemini Strategist Agent evaluation
    archetypes = {
    ...
    # Generate additional standard baseline accounts
    company_names = [
    ...
    for i, name in enumerate(company_names, start=1004):
        archetypes[name] = {
    ...
    # Monthly snapshots over 24 months (2024-09 to 2026-08)
    ...
    for company_name, config in archetypes.items():
        ...
        for m in range(months):
            ...
            snapshot_date = (base_date + timedelta(days=m*30.5)).strftime("%Y-%m-01")

            if profile == "upsell":
                # Rapid growth: seat utilization > 95%, high API volume, zero tickets
                mau = int(seats * (0.85 + (m / months) * 0.14))  # Grows to ~99% seat saturation
                api_volume = 150000 + (m * 12000) + random.randint(-2000, 2000)
                adv_features = True
                tickets = 0 if m > 12 else random.choice([0, 1])

            elif profile == "churn_risk":
                # Drop in active users, spike in critical support tickets
                decay_factor = max(0.3, 1.0 - (m / months) * 0.5)  # Usage drops 50%
                mau = int(seats * 0.75 * decay_factor)
                api_volume = int(300000 * decay_factor) + random.randint(-5000, 5000)
                adv_features = False
                tickets = random.randint(3, 8) if m >= 18 else random.randint(0, 2)

            elif profile == "stagnant":
                # Low engagement: underutilized seats, basic features only
                mau = int(seats * 0.25) + random.randint(-2, 2)
                api_volume = random.randint(5000, 12000)
                adv_features = False
                tickets = random.choice([0, 0, 1])

            else:  # standard random baseline
    ...
    # Write output to CSV
    fieldnames = [
        "account_id", "company_name", "snapshot_month", "active_users",
        "allocated_seats", "api_call_volume", "advanced_features_enabled",
        "critical_support_tickets", "contract_tier"
    ]
